...
using static DialogueEditor.UIConversationButton;
...
namespace DialogueEditor
{
    public class ConversationManager : MonoBehaviour
    {
        ...
        public RectTransform NextEndPanel;
        // Dialogue UI
        ...
        public UIConversationButton NextEndPrefab;
        // Default values
        ...
        private UIConversationButton uiNextEndButton;
        ...
        private void TurnOnUI()
        {
            ...
            NextEndPanel.gameObject.SetActive(true);
        ...
        private void TurnOffUI()
        {
            ...
            NextEndPanel.gameObject.SetActive(false);
            SetState(eState.Off);
#if UNITY_EDITOR
        ...
        private void CreateUIOptions()
        {
            ...
            if (m_currentSpeech.ConnectionType == Connection.eConnectionType.Option)
            {
                for (int i = 0; i < m_currentSpeech.Connections.Count; i++)
                {
                    ...
                    {
                        UIConversationButton uiOption = CreateButton(UIConversationButton.eButtonType.Option);
            ...
            else
            {
                ...
                UIConversationButton uiNextEnd = null;
                if (notAutoAdvance || allowVisibleOptionWithAuto)
                {
                    if (m_currentSpeech.ConnectionType == Connection.eConnectionType.Speech)
                    {
                        uiNextEnd = CreateButton(UIConversationButton.eButtonType.Speech);
                        SpeechNode next = GetValidSpeechOfNode(m_currentSpeech);

                        // If there was no valid speech node (due to no conditions being met) this becomes a None button type
                        if (next == null)
                        {
                            uiNextEnd.SetupButton(UIConversationButton.eButtonType.End, null, endFont: m_conversation.EndConversationFont);
                            uiNextEnd.SetImage(EndButton, NextEndPanel);
                        }
                        // Else, valid speech node found
                        else
                        {
                            uiNextEnd.SetupButton(UIConversationButton.eButtonType.Speech, next, continueFont: m_conversation.ContinueFont);
                            uiNextEnd.SetImage(NextButton, NextEndPanel);
                        }

                    }
                    else if (m_currentSpeech.ConnectionType == Connection.eConnectionType.None)
                    {
                        uiNextEnd = CreateButton(UIConversationButton.eButtonType.End);
                        uiNextEnd.SetupButton(UIConversationButton.eButtonType.End, null, endFont: m_conversation.EndConversationFont);
                        uiNextEnd.SetImage(EndButton, NextEndPanel);
                    }
                }
                uiNextEndButton = uiNextEnd;
        ...
        private void ClearOptions()
        {
            ...
            if (uiNextEndButton != null)
            {
                GameObject.Destroy(uiNextEndButton.gameObject);
            }
        }
        ...
        private UIConversationButton CreateButton(UIConversationButton.eButtonType buttonType)
        {
            UIConversationButton button = null;

            if (buttonType == UIConversationButton.eButtonType.Option)
            {
                button = GameObject.Instantiate(ButtonPrefab, OptionsPanel);
                m_uiOptions.Add(button);
            }
            else if (buttonType == UIConversationButton.eButtonType.Speech || buttonType == UIConversationButton.eButtonType.End)
            {
                button = GameObject.Instantiate(NextEndPrefab, NextEndPanel);
            }
